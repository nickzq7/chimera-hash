...
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.18823652.svg)](https://doi.org/10.5281/zenodo.18823652)

**Author:** Manish Kumar Parihar — Independent Researcher, 2026  
**Paper:** [CHIMERA_Final_Paper.pdf](CHIMERA_Final_Paper.pdf)  
**Zenodo:** [doi.org/10.5281/zenodo.18823652](https://doi.org/10.5281/zenodo.18823652)  
...
**CHIMERA wins 6/16 categories — exactly where TF-IDF fails hardest.** TF-IDF wins where topic/vocabulary discrimination is the dominant signal. CHIMERA wins where character-level structural similarity matters more than exact word matching.
...
Standard TF-IDF requires a reference corpus of millions of documents:
```
IDF(t) = log(|D| / df(t))     ← needs a corpus
```

Chaos-IDF replaces this entirely with the logistic map — a well-studied equation from chaos theory. Given a character at ASCII code `c` and position `p`:
...
At r = 3.9 the map is fully chaotic. Two initial conditions differing by ε diverge to ≈4.9ε after 3 iterations. Common character-position combinations cluster near periodic orbits (moderate weight). Rare combinations produce extreme x₃ values (high discriminative weight). **No corpus required, ever.**
...
Every prior multi-component similarity algorithm treats its modules independently. CHIMERA couples them: the SimHash-SRP bit-signature is XOR-folded into the genome seed for the chaotic attractor trajectory.

```
genome_seed = XOR_fold(simhash_signature[:64])
# Attractor trajectory initialised from the structural hash
# Texts similar in LSH space → similar attractor trajectories
# Static signal and dynamic signal reinforce each other
```

No prior publication couples LSH output to chaotic attractor initialisation.

### 3. Instant-NGP multiresolution encoding applied to text (novel application)

Müller et al. 2022 introduced multiresolution hash grids for 3D neural scene rendering. CHIMERA applies this to 1D character positions in text for the first time:
...
Every input text converges to `STABLE_ATTRACTOR` at Level 5, regardless of content type:
...
This convergence guarantee has no equivalent in any prior text fingerprinting algorithm.
...
# Fast mode — SimHash-SRP only (0.82 ms/pair), for pre-filtering at scale
...
print(f"Genome seed: {fp['genome_seed']}")

# View attractor convergence level by level
...
> Small floating-point differences from paper values are normal across Python versions. Core rankings and attractor convergence are stable.

Runtime: ~3 minutes on any laptop CPU. No GPU required.
...
### Limitations (documented in paper Section 6.4)
...
| Negation detection | NLI model | No dedicated negation signal in v1 |
| Scale > 100M pairs/day | SimHash, MinHash | 3.7ms vs 0.1ms = 37× slower |

### Recommended hybrid pipeline (paper Section 6.3)
...
*Evaluated on 16-pair development set. v4 uses the full 115-pair benchmark.
...
All 115 test pairs are in Appendix A of the paper and in `run_benchmark.py`. Every score is computed live at runtime. No values are pre-stored or manually adjusted.
